...
struct aiocb **iocb_free;	/* array of pointers to iocb */
...
int init_iocb(int n, int iosize)
{
	...
	if ((iocb_free = malloc(n * sizeof(struct aiocb *))) == 0) {
		return -1;
	}

	for (i = 0; i < n; i++) {
		if (!(iocb_free[i] = (struct aiocb *) malloc(sizeof(struct aiocb))))
			return -1;
...
void free_iocb(struct aiocb *io)
{
 ...
 */
static void wr_done(io_context_t ctx, struct  aiocb *iocb, long res, long res2)
{
	if (res2 != 0) {
 ...
 */
static void rd_done(io_context_t ctx, struct aiocb *iocb, long res, long res2)
{
